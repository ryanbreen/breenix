...
SwiftPM command-line tooling for preserving Breenix gate evidence and printing a host-facts trace for ARM (local QEMU) and x86 (beast build environment) gate runs.
...
swift run breenix-runs list
swift run breenix-runs list --arch aarch64 --verdict fail
...
`run x86` drives `docker/qemu/run-x86-gate.sh` on the beast build environment (`<x86-build-environment>`, the only x86 build/boot host per `[[beast-x86-build-host]]`) rather than composing a QEMU command line locally: it makes a private `git clone --shared` of the container's canonical `<canonical-checkout>` checkout at the sha under test (default: this repo's local HEAD; pass `--sha` to override), points `BREENIX_GATE_TMP` inside that clone, streams the gate's build+boot output back live, pulls the resulting evidence back over the same ssh channel, and removes the clone as its final teardown step on both a passing and a failing gate exit. `--sha` defaults to the local HEAD, so the branch under test must already be pushed to `origin` before running this — beast's canonical checkout fetches from GitHub, not from this Mac. A dirty local working tree prints a warning (beast tests the pushed commit, not uncommitted changes) rather than failing. `--dry-run` builds and prints that same plan (the four ssh/remote-environment command lines) without running any of it: no ssh connection is opened and the run store's index is left unchanged. `--boots`/`--mode` map directly onto the gate script's own `[count] [mode]` arguments; `boot-tests` and `prod` (DESIGN.md 2.3's other two x86 CLI-surface entries) are not implemented by this PR.
...
6 of 34 `docker/qemu/run-*.sh` scripts (`run-aarch64-boot-test-strict.sh`, `run-aarch64-testing-profile-boot-test.sh`, `run-aarch64-prod-profile-boot-test.sh`, `run-x86-gate.sh`, `run-x86-boot-tests.sh`, `run-x86-prod-profile-boot-test.sh`) call into the store automatically once their own verdict is final, via `docker/qemu/lib/run-inspector-import.sh` — you do not have to run `import` by hand after one of these gates (see DESIGN.md §4.5 for the full list and why the others don't). That hook writes a `run-inspector.json` sidecar into the evidence directory carrying the gate's own verdict, exit code, and a freshly minted UUID (`GateProvenance`, `DESIGN.md` §4.5), then attempts a bounded (default 15s, `BREENIX_RUNS_IMPORT_TIMEOUT`-controlled) local `breenix-runs import` of that directory. Because the store's ID for such a run is derived from that UUID rather than from serial content, importing the same directory again — by hand, or because the in-gate attempt timed out — is a no-op, not a duplicate. Set `BREENIX_RUNS_NO_IMPORT=1` to disable the hook; a gate never fails because this step failed or was skipped.
