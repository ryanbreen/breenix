...
fn main() -> Result<()> {
    match Cmd::from_args() {
        ...
        Cmd::Arm64BootStages => arm64_boot_stages(),
...
fn get_arm64_boot_stages() -> Vec<BootStage> {
    vec![
        ...
        BootStage {
            ...
            marker: "USERSPACE CLOCK_GETTIME: OK",
            failure_meaning: "Userspace process called clock_gettime syscall but got zero time or syscall failed",
            check_hint: "Verify SVC dispatch to SYS_clock_gettime (228) works from EL0 and returns non-zero time",
        },
        BootStage {
            name: "Userspace brk syscall validated",
            marker: "USERSPACE BRK: ALL TESTS PASSED",
...
/// Boot ARM64 kernel and validate each boot stage sequentially.
///
/// ARM64 uses a single UART serial output (PL011 at 0x09000000) mapped to one file.
/// Unlike x86_64 which has dual COM1/COM2, all ARM64 output goes to one serial port.
///
/// This function:
/// 1. Builds userspace test binaries for aarch64
/// 2. Creates ext2 disk image with test binaries
/// 3. Builds ARM64 kernel (optionally with testing features)
...
fn arm64_boot_stages() -> Result<()> {
    ...
    // Step 1: Build userspace test binaries for aarch64
    ...
    if ext2_img.exists() {
        ...
        if userspace_dir.exists() {
            // Ensure aarch64 output directory exists
        ...
        if create_disk_script.exists() {
            // On Linux CI, this needs sudo for mount; on macOS it uses Docker
    ...
    let kernel_binary = Path::new("target/aarch64-breenix/release/kernel-aarch64");
    if !kernel_binary.exists() {
        bail!("ARM64 kernel binary not found at {}", kernel_binary.display());
    }
    println!("  ARM64 kernel built successfully");

    // Step 4: Launch QEMU
    let serial_output_file = "target/arm64_boot_stages_output.txt";
    ...
    let mut child = Command::new("qemu-system-aarch64")
        ...
        .stderr(Stdio::inherit())
        .spawn()
        .map_err(|e| anyhow::anyhow!("Failed to spawn qemu-system-aarch64: {}", e))?;

    save_qemu_pid(child.id());

    // Wait for output file to be created
    let start = Instant::now();
    let file_creation_timeout = Duration::from_secs(30);
    ...
    // Step 5: Monitor output and validate stages
    let mut stages_passed = 0;
    let mut last_content_len = 0;
    let mut checked_stages: Vec<bool> = vec![false; total_stages];
    let mut stage_timings: Vec<Option<StageTiming>> = vec![None; total_stages];
    ...
    let timeout = if std::env::var("CI").is_ok() {
        ...
        Duration::from_secs(300) // 5 minutes locally
    };
    ...
    while test_start.elapsed() < timeout {
        // Read serial output (ARM64 has single UART, all output goes to one file)
...
fn boot_test_btrt(arch: &str) -> Result<()> {
    ...
    if is_arm64 {
        ...
        // Ensure ext2 disk image exists (ext2 is arch-independent)
        let ext2_img = Path::new("target/ext2-aarch64.img");
        if !ext2_img.exists() {
            let src = Path::new("testdata/ext2.img");
            if src.exists() {
                fs::copy(src, ext2_img).context("Failed to copy ext2 image for ARM64")?;
                println!("  Copied ext2 disk from testdata/ext2.img");
            } else {
                bail!("ext2 disk image not found at testdata/ext2.img. Create one first.");
            }
        }
                    ...
                    if line.contains("[btrt]") && line.contains("phys") {
                        ...
                        if let Some(addr_str) = line.split("phys ").nth(1) {
                            if let Some(addr_hex) = addr_str.split_whitespace().next() {
                                ...
                                btrt_phys_addr = u64::from_str_radix(cleaned, 16).ok();
                            }
                        }
