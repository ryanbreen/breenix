...
# script's own port began: every gate builds its scratch paths under one
# overridable base so two concurrent lanes stop clobbering each other. The
# default keeps the paths this script's parked-branch original used
# byte-for-byte, and the two per-gate overrides below still win when set.
# claim-lint:ok: the pattern is copied from
# docker/qemu/run-x86-boot-tests.sh:79-87 -- see
# docs/planning/green-program/gates/GATE-TMP-BASEDIR-2026-09-05.md
BREENIX_GATE_TMP="${BREENIX_GATE_TMP:-/tmp}"
# Must be absolute: OUTPUT_ROOT is read both before and after the `cd` into
# $BREENIX_ROOT that the build step performs, and a relative value would
# resolve differently in each place (review finding F6 on #797).
case "$BREENIX_GATE_TMP" in
    /*) ;;
    *) echo "Testing-profile gate: FAIL (BREENIX_GATE_TMP must be an absolute path, got: $BREENIX_GATE_TMP)" >&2; exit 1 ;;
esac
OUTPUT_ROOT="${BREENIX_TESTING_PROFILE_OUTPUT_DIR:-$BREENIX_GATE_TMP/breenix_aarch64_testing_profile}"
FAILURE_ROOT="${BREENIX_TESTING_PROFILE_FAILURE_DIR:-$BREENIX_GATE_TMP/breenix_testing_profile_failures}"
