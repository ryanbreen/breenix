...
pub fn test_minimal_userspace() {
        ...
        if let Some(manager) = manager.as_mut() {
            ...
            if let Some(process) = manager.get_process_mut(crate::process::ProcessId::new(1)) {
                if let Some(ref mut page_table) = process.page_table {
                    ...
                    page_table
                        .map_page(user_page, frame, flags)
                    ...
                    if let Some(phys_addr) = page_table.translate_page(user_page.start_address()) {
                        if phys_addr == frame.start_address() {
                            crate::serial_println!(
                                "✓ Mapped int3 at 0x400000 in process page table -> {:#x}",
                                frame.start_address().as_u64()
                            );
                        } else {
                            crate::serial_println!(
                                "ERROR: page mapped to wrong frame in process table!"
                            );
                        }
                    } else {
                        crate::serial_println!(
                            "ERROR: page not mapped in process table after map_page!"
                        );
                    }
            ...
            if let Some(process) = manager.get_process_mut(crate::process::ProcessId::new(1)) {
                if let Some(ref mut page_table) = process.page_table {
                    ...
                    let flags = PageTableFlags::PRESENT
                        ...
                        | PageTableFlags::NO_EXECUTE;
                    use x86_64::PhysAddr;
                    let stack_frame = x86_64::structures::paging::PhysFrame::containing_address(
                        PhysAddr::new(stack_frame_addr),
                    );
                    page_table
                        .map_page(stack_page, stack_frame, flags)
    ...
    // This is critical - without it, CPU can't switch stacks on exception
    {
        ...
        if actual_rsp0 != kernel_stack_top.as_u64() {
            panic!(
                "TSS.RSP0 not set correctly! Expected {:#x}, got {:#x}",
