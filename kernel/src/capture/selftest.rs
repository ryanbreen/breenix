...
//! module contributes nothing to any shipped or gated kernel. Its purpose is
//! to give every later PR in the failure-capture program a capture it can
//! test against on demand, instead of waiting for the rare fault the real
//! edges fire on.
//!
...
//! context would be a weaker rehearsal: it would never exercise the
...
/// scheduler all have real content to report. Well inside every boot window
/// this runs in: the aarch64 strict gate polls to ~18 s and the x86
/// boot-tests gate runs longer still.
