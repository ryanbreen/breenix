...
impl Work {
    ...
    pub fn wait(&self) {
        ...
        // Note: TID 0 is valid (it's the idle/boot thread), so we must use Option properly
        let tid = match scheduler::current_thread_id() {
            Some(id) => id,
            None => {
                // No valid thread ID (early boot before scheduler init) - fall back to spin loop
