...
pub fn test_rtc_and_real_time() {
    ...
    match crate::time::rtc::read_rtc_time() {
        Ok(timestamp) => {
            ...
            log::info!("RTC DateTime: {:04}-{:02}-{:02} {:02}:{:02}:{:02}",
    ...
    log::info!("Boot time: {:04}-{:02}-{:02} {:02}:{:02}:{:02}",
              boot_dt.year, boot_dt.month, boot_dt.day,
              boot_dt.hour, boot_dt.minute, boot_dt.second);
    ...
    log::info!("Monotonic time: {} ms ({} seconds since boot)",
              monotonic_ms, monotonic_ms / 1000);
    ...
    log::info!("Real time after wait: {:04}-{:02}-{:02} {:02}:{:02}:{:02}",
              real_time2.year, real_time2.month, real_time2.day,
              real_time2.hour, real_time2.minute, real_time2.second);
