...
#[cfg(target_arch = "aarch64")]
static F20C_END_BOOT_AUDIT_DUMPED: core::sync::atomic::AtomicBool =
    core::sync::atomic::AtomicBool::new(false);

/// F20c diagnostic: emit a one-shot per-CPU timer/idle snapshot after the
/// `--test 45` dwell has had time to expose CPU0 WFI behavior.
#[cfg(target_arch = "aarch64")]
fn spawn_f20c_end_boot_audit_threads() {
    let online = kernel::arch_impl::aarch64::smp::cpus_online()
        .max(1)
        .min(kernel::arch_impl::aarch64::constants::MAX_CPUS as u64);

    for _ in 0..online {
        if kernel::task::kthread::kthread_run(f20c_end_boot_audit_thread, "f20c_audit").is_err() {
            serial_println!("[F20C] failed to spawn end-of-boot audit thread");
        }
    }
}

#[cfg(target_arch = "aarch64")]
fn f20c_end_boot_audit_thread() {
    let freq = kernel::arch_impl::aarch64::timer::frequency_hz();
    let wait_ticks = freq.saturating_mul(40);
    ...
    if !F20C_END_BOOT_AUDIT_DUMPED.swap(true, core::sync::atomic::Ordering::AcqRel) {
        f20c_dump_end_boot_audit();
    }
}

#[cfg(target_arch = "aarch64")]
fn f20c_dump_end_boot_audit() {
    use core::sync::atomic::Ordering;
    use kernel::arch_impl::aarch64::context_switch::{
        F20C_LAST_IDLE_ARM_TICK, F20C_LAST_IDLE_ARM_TSC, F20C_LAST_POST_WFI_TICK,
        F20C_POST_WFI_COUNT,
    };
    ...
    serial_println!(
        ...
        F20C_LAST_IDLE_ARM_TICK[0].load(Ordering::Relaxed),
        F20C_LAST_IDLE_ARM_TICK[1].load(Ordering::Relaxed),
        F20C_LAST_IDLE_ARM_TICK[2].load(Ordering::Relaxed),
        F20C_LAST_IDLE_ARM_TICK[3].load(Ordering::Relaxed),
        F20C_LAST_IDLE_ARM_TICK[4].load(Ordering::Relaxed),
        F20C_LAST_IDLE_ARM_TICK[5].load(Ordering::Relaxed),
        F20C_LAST_IDLE_ARM_TICK[6].load(Ordering::Relaxed),
        F20C_LAST_IDLE_ARM_TICK[7].load(Ordering::Relaxed),
    );
    serial_println!(
        "[END_OF_BOOT_AUDIT] idle_arm_tsc=[{},{},{},{},{},{},{},{}]",
        F20C_LAST_IDLE_ARM_TSC[0].load(Ordering::Relaxed),
        F20C_LAST_IDLE_ARM_TSC[1].load(Ordering::Relaxed),
        F20C_LAST_IDLE_ARM_TSC[2].load(Ordering::Relaxed),
        F20C_LAST_IDLE_ARM_TSC[3].load(Ordering::Relaxed),
        F20C_LAST_IDLE_ARM_TSC[4].load(Ordering::Relaxed),
        F20C_LAST_IDLE_ARM_TSC[5].load(Ordering::Relaxed),
        F20C_LAST_IDLE_ARM_TSC[6].load(Ordering::Relaxed),
        F20C_LAST_IDLE_ARM_TSC[7].load(Ordering::Relaxed),
    );
    serial_println!(
        "[END_OF_BOOT_AUDIT] post_wfi_count=[{},{},{},{},{},{},{},{}]",
        F20C_POST_WFI_COUNT[0].load(Ordering::Relaxed),
        F20C_POST_WFI_COUNT[1].load(Ordering::Relaxed),
        F20C_POST_WFI_COUNT[2].load(Ordering::Relaxed),
        F20C_POST_WFI_COUNT[3].load(Ordering::Relaxed),
        F20C_POST_WFI_COUNT[4].load(Ordering::Relaxed),
        F20C_POST_WFI_COUNT[5].load(Ordering::Relaxed),
        F20C_POST_WFI_COUNT[6].load(Ordering::Relaxed),
        F20C_POST_WFI_COUNT[7].load(Ordering::Relaxed),
    );
    serial_println!(
        "[END_OF_BOOT_AUDIT] post_wfi_tick=[{},{},{},{},{},{},{},{}]",
        F20C_LAST_POST_WFI_TICK[0].load(Ordering::Relaxed),
        F20C_LAST_POST_WFI_TICK[1].load(Ordering::Relaxed),
        F20C_LAST_POST_WFI_TICK[2].load(Ordering::Relaxed),
        F20C_LAST_POST_WFI_TICK[3].load(Ordering::Relaxed),
        F20C_LAST_POST_WFI_TICK[4].load(Ordering::Relaxed),
        F20C_LAST_POST_WFI_TICK[5].load(Ordering::Relaxed),
        F20C_LAST_POST_WFI_TICK[6].load(Ordering::Relaxed),
        F20C_LAST_POST_WFI_TICK[7].load(Ordering::Relaxed),
    );
    serial_println!(
...
pub extern "C" fn kernel_main(hw_config_ptr: u64) -> ! {
    ...
    #[cfg(target_arch = "aarch64")]
    spawn_f20c_end_boot_audit_threads();
