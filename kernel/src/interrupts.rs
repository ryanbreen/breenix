...
extern "x86-interrupt" fn general_protection_fault_handler(
    ...
    // For userspace GPFs (e.g., CLI/HLT privilege violations), terminate the process gracefully
    if from_userspace {
        log::info!("Terminating userspace process due to privilege violation...");

        // Get CR3 to find the faulting process
        ...
        crate::process::with_process_manager(|pm| {
            if let Some((pid, process)) = pm.find_process_by_cr3_mut(cr3) {
                let name = process.name.clone();
                faulting_thread_id = process.main_thread.as_ref().map(|t| t.id);
                log::info!("Killing process {} (PID {}) due to GPF (CR3={:#x})",
                    name, pid.as_u64(), cr3);
                pm.exit_process(pid, -11); // SIGSEGV exit code
            } else {
                log::error!("Could not find process with CR3={:#x} - cannot terminate", cr3);
