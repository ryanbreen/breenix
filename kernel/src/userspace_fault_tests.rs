...
pub fn create_unmapped_access_test_elf() -> Vec<u8> {
    ...
    elf.extend_from_slice(&[
        ...
        0x48, 0xb8, 0x00, 0x00, 0x00, 0x50, 0x00, 0x00, 0x00,
        0x00, // mov rax, 0x50000000 (unmapped)
...
pub fn run_fault_tests() {
    ...
    #[cfg(feature = "testing")]
    {
        ...
        log::info!(
            "These tests prove Ring 3 privilege isolation by attempting privileged operations"
        );
        ...
        match crate::process::create_user_process(
            String::from("unmapped_fault_test"),
            &unmapped_elf,
        ) {
            Ok(pid) => {
                log::info!(
                    "  Created unmapped access test process PID {}",
                    pid.as_u64()
                );
