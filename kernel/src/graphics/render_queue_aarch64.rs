//! Deferred framebuffer rendering queue for ARM64.
//!
//! This module provides a ring buffer for queueing text output that will be
//! rendered to the framebuffer by a dedicated kernel thread. This is needed
//! for ARM64 because the TTY echo path runs in interrupt context, where it's
//! unsafe to call into the complex terminal_manager rendering stack.
//!
//! ## Architecture
//!
//! ```text
//! Interrupt context             Render thread (own stack)
//! ─────────────────             ────────────────────────
//!        │                              │
//!  queue_byte()  ──────────────►  drain_and_render()
//!  (shallow stack)               (calls terminal_manager)
//!        │                              │
//!   Ring Buffer ◄──────────────────────►│
//! ```
//!
...
//! mutex overhead that could cause issues in interrupt context.
...
/// 8KB is enough for several screens of text.
...
pub fn init() {
    ...
    log::info!("ARM64 render queue initialized ({}KB buffer)", QUEUE_SIZE / 1024);
}
...
///
/// This function is designed to be very cheap and safe for interrupt context.
/// It uses try_lock semantics - if the lock is busy, it returns false.
#[inline]
pub fn queue_byte(byte: u8) -> bool {
    ...
    super::render_task_aarch64::wake_render_thread();
