...
//!
//! A replay is keyed by `(root_seed, component, dcpu, iteration)`, and `dcpu`
//! is reported because it is an unseeded domain input. Within that domain:
//!
//! * `Pen` exactly repeats the driver's draws and release cadence; whether peer
//!   rendezvous form at the same time and what interrupts meet each seam do not.
//! * `Adversarial` exactly repeats each CPU's own draw and operation sequence;
//!   peers cannot perturb one another's streams, but scheduling decides how far
//!   each peer advances and what kernel state each operation meets.
//! * `Ambient` exactly repeats the driver's draws only. The cohort activity,
//!   timer phase, victim set, host-vCPU scheduling and virtio timing are not
//!   generated by this PRNG and are not replayed.
//!
//! The eight-second wall-clock budget also admits a different final iteration
//! each boot. On the gate profile — `-smp 4` multi-threaded TCG — a seed thus
//! replays *what the harness asks at a named iteration*, not *what the machine
//! does in response*. M1 can replay reliably because its catch is manufactured
//! wholly inside one masked probe critical section. M4 needs a `pending_next`
//! handoff produced by the boot's own concurrent scheduling, which the seed
//! does not control. Replay hit rate is therefore measured, never asserted.
//!
...
pub const ALL_OPS: &[AntagonistOp] = &[
    AntagonistOp::Unblock,
    AntagonistOp::Placement,
    AntagonistOp::KernelSchedule,
...
pub fn draw(root_seed: u64, component: u8, cpu: u8, iteration: u64) -> DrawVector {
    ...
    DrawVector {
        ...
        // Round 2 deliberately changes the stream shape from round 1's binary
        // `& 1` choice: every paired operation is selected from this census.
        antagonist_op: ALL_OPS[(rng.next_u64() % ALL_OPS.len() as u64) as usize],
