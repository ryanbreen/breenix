...
use crate::arch_impl::current::constants::{
    DISPATCH_MARK_INVALID, DISPATCH_MARK_VALID, PERCPU_CPU_ID_OFFSET, PERCPU_CURRENT_THREAD_OFFSET,
    PERCPU_DISPATCH_MARK_RIP_OFFSET, PERCPU_DISPATCH_MARK_RSP_OFFSET,
    PERCPU_DISPATCH_MARK_STATE_OFFSET, PERCPU_DISPATCH_MARK_TID_OFFSET,
    PERCPU_DISPATCH_MARK_WAIT_ITERS_OFFSET, PERCPU_EXCEPTION_CLEANUP_CONTEXT_OFFSET,
...
pub fn can_schedule(saved_cs: u64) -> bool {
    ...
    // Match the interrupt-return switch gate before the blocked/exception arms:
    // a kernel continuation holding the count cannot donate its CPU. Completion
    // must preserve boot's count; testing count > 0 is not a syscall predicate.
    if !returning_to_userspace && current_preempt & 0x0fff_ffff != 0 {
        return false;
    }
    ...
    // Parked threads need dispatch even without a fresh quantum request.
    // 673 / 718 added BlockedOnTimer; retain that admission set here.
    // BlockedOnIO continues through the count-zero need_resched arm below.
    // Unlike the old expression alone, the early kernel-count guard above
    // also covers these blocked and exception-cleanup arms. It matches the
    // later interrupt-return guard rather than relying on that second check.
    //
    // 508's idle boot wait is different from a parked worker: Completion
    // retains its caller's count and waits for an IRQ without publishing idle
    // as BlockedOnIO. The old Completion predicate mistook boot's held count
    // for syscall context and released it, admitting an idle-kernel switch.
