...
| file | what it is |
|---|---|
| `01-aarch64-prod-build.log` | aarch64 production build, `aarch64-breenix-kernel.json` (the soft-float kernel target — never the NEON one, per the #528 re-arm rule), after a forced `cargo clean -p kernel --release`. Exit 0. The only `warning:` lines are the pre-existing toolchain `core` future-incompatibility note, which is not project code. |
...
| `04-ss25-summary.log` | service-sequence gate, 25 boots × 2 profiles (`max`, `cortex-a72`). Both profiles PASSED, 50/50 GREEN, every named bucket 0, `Resume PC refused: 0/50`, `RET dispatch refused: 0/50`. Per-boot lines elided; the gate's own preserved serials are under `/tmp/breenix_aarch64_service_sequence_gate_*`. |
| `05-strict20-summary.log` | strict boot gate, 1×20. 20/20, 100%, 203 s. Includes the gate's `kernel_no_neon_guard` preflight: 0 FP/SIMD load/store instructions in kernel `.text`. |
| `06-x86-boot-tests-beast.log` | x86 custody boot tests on beast, 4 boots (one run of 1, then a run of 3). Every boot `x86 userspace gate: PASS - exited=102 expected>=101 nonzero=0 allowlist=0` and `x86 frame-custody gate run N: PASS`. No reds this round, so nothing to attribute. |
