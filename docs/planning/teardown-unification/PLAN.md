...
| **DEBT-3 — the blocking-primitive inventory was NOT closed at nine; it is now closed and enumerated at TEN, and the register's own instruction on one of the four sites was WRONG** | **P9** | **Status: the inventory half of this debt is discharged; the interlock half is not.** Re-verified at `main` @ `102317b4` and closed by the **#580** PR, which corrects this row rather than quietly dropping it. **(1)** `kernel/src/syscall/futex.rs:115` — **already closed before that PR**, by `16d1a062` (PR #604): FUTEX_WAIT publishes through `WaitQueueHead::prepare_to_wait_checked` → `Scheduler::block_current_for_io_with_timeout`. The `futex.rs:115` citation is **withdrawn**. **(2)** the direct `BlockedOnIO` publication (cited here as `scheduler.rs:2607`; the anchor had drifted twice) — the write now lives in the private `Scheduler::block_current_for_io_publish`, reached only through `block_current_for_io_with_timeout`; `WaitQueueHead::prepare_to_wait` calls that primitive and the out-of-family `pub fn publish_current_io_wait_state` is deleted. Disclosed delta: the plain I/O wait path now also runs the defensive per-CPU ready-queue scrub that `prepare_to_wait_checked` already ran. **(3)** `kthread_park()` (`task/kthread.rs`) — now calls `Scheduler::block_current()` inside its existing `without_interrupts` block instead of writing `thread.state` inline; it thereby also picks up the CPU-tick charge the park path had been skipping. *Disclosed, not fixed here (**#647**):* `block_current` publishes `Blocked` but does not remove the thread from the ready queue and does not set `blocked_in_syscall`, so all 14 callers open-code one or both afterwards — a real incompleteness of the primitive, and exactly the shape P9's interlock has to install a guard into, but folding it in would change 14 call sites at once. **(4)** the "dead two-level pair" — **this row's claim was FALSE when it was written, not merely stale, and its prescription ("delete the pair") would have deleted a live primitive.** `Scheduler::block_current` had **14 production callers** at `2c7b8798` and still has them (`syscall/handlers.rs` ×7, `syscall/socket.rs` ×4, `syscall/fs.rs`, `net/loopback_pump.rs`, `test_framework/registry.rs`); `Thread::set_blocked` was its only caller in turn. What was actually wrong was **two stale `#[allow(dead_code)]` attributes on live code** — a zero-tolerance violation in its own right. The #580 PR therefore **removes both suppressions**, inlines the write into `block_current` and deletes only `Thread::set_blocked`, the differently-named mutator; forced-clean builds of both aarch64 profiles with zero project warnings are the evidence the suppressions were stale. **(5) — new, not in #580 as filed:** the family census had a **`pub(crate)` blind spot**. `validate_blocking_primitives` filtered prefix-matched definitions through `preceded_by_keyword(…, "pub")`, and the code byte before `fn` in `pub(crate) fn` is `)`, so `WaitQueueHead::prepare_to_wait_checked` — a genuine blocking primitive — was invisible to the ratchet that exists to forbid exactly that. The filter is dropped; the inventory is **ten entry points** (eight `Scheduler::block_current*`, `prepare_to_wait`, `prepare_to_wait_checked`) and **eleven census rows** once the private `block_current_for_io_publish` is counted. **What the #580 PR discharges:** the inventory is closed and enumerated; every `ThreadState::Blocked*` publication in `kernel/src` outside a `#[cfg(test)]` fixture now sits inside a family-named primitive; and the ratchet gains three census-shaped rules that see the **class** rather than the sites — census A (blocked-state publications), a derived rule requiring every production census-A row to be family-named, and census B (opaque `state` field stores, the laundering shape). **What P9 still owes:** the no-new-block admission interlock itself, and therefore §1.5's one-way-door claim. That bullet is carried forward to P9's acceptance, citing the #580 PR as its precondition — the interlock now installs in one place per family instead of being spread across the tree | v3 pre-check and re-check, item **D NOT-CLOSED** (both passes). Surface widened and re-anchored by the 2026-08-15 tranche-2 re-ratification (§3.2), tracked as **#580**; sites 1-5 re-verified at `102317b4` and the deadness claim retracted by the #580 PR. DESIGN §0.3 (closure D), §0.3.1, §1.5; PLAN P0 rule 2, P9 |
| **DEBT-4 — the x86 reap path bypasses the tombstone gate, and the blast radius is 24 call sites, not four** | **P6a** | P6a's whole claim is that a row is removed only by the **two-event join** (`reaped` ∧ `retired`, whichever writer sees the other flag set performs the removal). **`kernel/src/syscall/handlers.rs:3128`** (re-derived at `174e5227`; the design said `:3101`, the v3 register said `:3123`) removes the row directly on the live x86 reap path, inside `complete_wait` at `:3064`, and its **byte-similar duplicate** at `kernel/src/syscall/wait.rs:386` (inside `complete_wait` at `:335`) does the same — so the join is not the only remover and the retention gate can pass on aarch64 while x86 still frees a row out from under an un-retired receipt. **The choke point is real and the join still installs in one place:** `ProcessManager::remove_process` is **nine lines at `manager.rs:1342-1350`** — not the "four-line choke point at `:1086-1090`" this row used to claim, and not the "eleven lines at `:1342-1352`" a 2026-08-20 correction pass claimed; it now also clears `designated_init` via `record_init_designation_retired()` (P5a) as well as bumping `ROW_REMOVAL_EPOCH` through `note_process_row_removed()` (`task/process_task.rs:355-356`, static `:269`), so **both arches are still covered by one edit** rather than a per-call-site chase, and `tests/teardown_structure.rs` already pins that premise four ways: `ROW_REMOVAL_EPOCH_BUMPS` (`:2667-2669`), `DESIGNATED_INIT_WRITES` (`:2451-2454`), `PROCESS_ROW_MAP_MUTATIONS` (`:2463-2474`, the `remove_process => remove` entry), and the body check at `:6713-6717`. **What is materially larger than this row ever recorded: there are 24 call sites, not four.** Twenty are in `kernel/src/tracing/providers/teardown.rs` (`1581, 1724, 1983, 2001, 2403, 2571, 2756, 2779, 3424, 3863, 4203, 4247, 4287, 4302, 4355, 4490, 4582, 4840, 4842, 5035`), all `boot_tests`-gated in-kernel oracles, and **each of the eight oracles that owns them emits a counter line pinned as a literal by a merge gate on one or both arches** — so a broken oracle is a red gate, not a quiet test failure. The four outside it are the two live reaps above, **`manager.rs:184`** (the P5a creation-failure retire of a provisional PID-1 reservation, comment `:174-177`: *"only `remove_process` clears `designated_init`"* — **not a reap, and it must be exempted from the join or it strands a permanent tombstone**), and `task/process_task.rs:2134` (the `p1_row_epoch_gate` harness, not a live reap). The liveness-query census is likewise **32 raw `self.processes.get*(&…)` sites in `manager.rs`** (9 immutable + 23 `get_mut`), not the nine a prior pass recorded — every one of them in scope for P6a's gate extra (c). Before P6a's tranche: the removal routes through the join **and** P6a's scope text enumerates the treatment of all 24 callers by class (reap / oracle reap stand-in / synthetic fixture / creation-failure retire), **or** P6a's retention gate is honestly re-scoped to aarch64 with the x86 divergence named in AC-12's evidence column, a ratchet pinning the call sites by census shape, and a stated phase that closes it. **#540** is an independent, unresolved evidence gap on the x86 leg — the x86 harness has no gate mode that exercises process exit/teardown with counter visibility (`mode=full` hangs; `completion.rs` has no x86_64 timeout) — so P6a's x86 retention claim may have no gate mode to execute in at all; it is named here as a gap, not as re-pin work. Scoping the gate narrowly to avoid tripping on any of this, without naming it, is not available. **DISCHARGED by P6a. (i)** landed with PR #616 (the four-class taxonomy above). **(ii)** landed with P6a PR-2: both live reaps — `syscall/wait.rs` (aarch64) and `syscall/handlers.rs` (x86_64) — now call the join's reap arm, the raw removal sits behind `take_row_unconditionally` with `remove_row_joined` as its only conditional path, and `tests/teardown_structure.rs::debt4_row_removal_routes_through_the_join_on_both_arches` is the occurrence-based census rule that no call site outside the join and the three named exempt classes reaches the row destructor — reddened under two independent single mutations (an unjoined destructor call added on a live reap path; the join's reap arm deleted). **(iii)** is restated in the #540 paragraph of the gate matrix: the x86 leg does execute the real reap in the `boot_tests` profile, every x86 counter claim P6a makes is therefore a `boot_tests`-profile claim, and the gate runs on beast only. **#540, #583, #546, #588, #572, #630 and #636 stay open**; P6a re-pins what it moves with a per-delta derivation and closes none of them | v3 pre-check and re-check, item **B NOT-CLOSED** (second half, both passes): *"P6a omits the live x86 reap at `kernel/src/syscall/handlers.rs:3101`, which still removes the row directly"*; anchors and closure cost re-derived by the 2026-08-15 re-ratification (§3.2), then corrected again by the tranche-3 ratification artifact (`TRANCHE3-RATIFICATION-2026-08-20.md` R3/R4/R5) and re-verified in full at `main` @ `174e5227`. PLAN P6a |
...
| **D** — wait-migration soundness | **P9** ships the **no-new-block admission interlock** inside all **ten** blocking primitives (latched request ⇒ refuse to block ⇒ `EINTR`; the count was nine until the #580 PR closed the census's `pub(crate)` blind spot — see DEBT-3), making the reachability classification a one-way door. **`BlockedOnSignal` (`pause`/`sigsuspend`) becomes its own subphase P10c**; the legacy-arm deletion moves to **P10d**. The missing hard edges are added to the graph | P9, P10a-d, §0 graph |
...
2. **Boot/regression gates — the current matrix, re-derived at `174e5227` (post-#604, post-#614).**
   ...
   - **aarch64 init service-sequence gate:** `docker/qemu/run-aarch64-service-sequence-gate.sh`,
     ...
     the gate condition rather than any list here, which goes stale. **Repaired by P6a PR-2,
     re-derived against the script at that head: the gate is now zero-tolerance.** Its per-profile
     condition fails on **`575`, `576`, `626`, `635`, `641`, `DATA_ABORT`, `CLONE_EXEC`, `STRAND`,
     `BOOT_TEST_FAIL`, `596`, `612`, `609`, `P5B` and `UNATTRIBUTED`**, plus a nonzero
     `RESUME_PC_REFUSED`, `PERCPU_STACK_ALIEN`, `CPU_IDENTITY_SPLIT` or `RET_STAGE_REFUSED` marker
     line count. Its own summary line states the rule outright: *"census-only: every non-GREEN bucket
     is gate-failing, with no exceptions, including the open #576, #626, #635 and #641 defects"*.
     **The `589` bucket is gone** (retired by the #609 round after #589 closed) and **the #609
     run-wide rate ceiling is gone** — `609` is a gate-failing bucket like any other. An attributed
     bucket is an attribution, not a tolerance: it names the issue a red belongs to and still fails
     the profile.
   ...
   - **What this matrix cannot do today, stated so no phase walks into it blind.** The kernel-merge
     ...
     represented nowhere in this matrix. **#540 is restated by P6a PR-2 and is narrower than it
     reads**: the script it names, `run-x86-gate.sh`, **does not exist in the tree**, and
     `docker/qemu/run-x86-boot-tests.sh` in the `boot_tests` profile *does* exercise process
     exit/teardown with counter visibility — `main.rs` calls `test_exec::test_waitpid()` under
     `cfg(all(testing, not(interactive)))`, so the x86 join arm is reached in the gate profile. What
     remains true, and what every x86 counter claim in this document is therefore scoped to: there is
     **no x86 production-profile teardown gate** (aarch64 has
     `run-aarch64-prod-profile-boot-test.sh`; x86 has no counterpart), `completion.rs` still carries
     no x86_64 timeout, and the x86 gate runs **on beast only**.

   **Operating flake law, restated under ruling R52 (P6a PR-2 repair; supersedes R17's amended
   form).** **Attribution is required; tolerance is not granted.** The green-gate arc exited on the
   operator's criterion — *no unexplained boot failures* — and that criterion is now enforced
   structurally: `UNATTRIBUTED` is a gate-failing bucket, so any red whose signature nobody can name
   blocks the merge and re-enters the arc. A red matching a filed, field-exact signature is
   **attributed**, which means the issue citation, the tally and a preserved serial go in the PR
   body — it does **not** mean the gate passes. The service-sequence gate fails on every non-GREEN
   bucket including the open #576, #626, #635 and #641, and the kernel-merge gate requires 100%.
   R17's five-signature tolerated set is retired: #589 is CLOSED (2026-08-21) and its bucket is gone;
   #609 is CLOSED (PR #632) and its rate ceiling is gone; #596 is fixed (PR #600). #512, #562, #612
   and #613 remain filed hard failures. The one surviving allowance is **#555's exact aarch64
   softirq-under-starvation signature, ≤2 retries** — it is the only retry allowance on the tree.
...
6. **Tier-1 byte-identity:** all five Tier-1 files (`syscall/handler.rs`, `syscall/time.rs`,
   ...
   **eleven** such suites (coordinator ruling R11, 2026-08-17 counted ten; `strand_handoff_structure.rs`
   ...
   Re-derived at P6a PR-2's head (the `174e5227` figures below it were stale by 69 tests):
   `tests/teardown_structure.rs` (**66**),
   `tests/context_restore_structure.rs` (**95**),
   ...
   `tests/exit_tally_structure.rs` (**6**), `tests/block_request_lifetime_structure.rs` (**12**),
   `tests/serial_line_atomicity_structure.rs` (**9**), `tests/signal_eintr_predicate_structure.rs`
   (**2**) and `tests/strand_handoff_structure.rs` (**37**, added by #614) — **341 tests**,
...
| `TOMBSTONE_JOIN{reap_second}`, `TOMBSTONE_JOIN{retire_second}` *(new, closure B)* | the two removal branches of the two-event join in `ProcessManager` | **producers landed in P6a PR-2**, which asserts **both** nonzero in one run — the boot gate's `[TOMBSTONE_JOIN_ORACLE:…]` line carries both deltas. The counters left the declaration-only region in the same PR |
| `EXIT_BLOCK_REFUSED{family}` *(new, closure D)* | the pre-block interlock inside the **ten** blocking primitives enumerated in DESIGN §0.3 closure D (the eight `Scheduler::block_current*` entries in `task/scheduler.rs`, plus `WaitQueueHead::prepare_to_wait` and `prepare_to_wait_checked` in `task/waitqueue.rs`). *No line anchors: every one this row used to carry had drifted by `102317b4`, and the set is pinned by census shape in `tests/teardown_structure.rs`, not by line.* | **declared zero until P9**; P9 drives it nonzero and **each P10x re-asserts its own family's value is still nonzero** — *v3 repair:* the interlock is permanent, so this counter never reaches 0; what falls to 0 per family is `EXIT_LEGACY_REMOTE_MARK{family}` |
...
2. *(closure D)* **The blocking-primitive set — the exact TEN** enumerated in DESIGN §0.3 closure D
   (eight public `Scheduler::block_current*` entries, `WaitQueueHead::prepare_to_wait`, and
   `WaitQueueHead::prepare_to_wait_checked`), plus the private in-family publisher
   `Scheduler::block_current_for_io_publish` as an eleventh census row. An eleventh *entry point*
   added without the interlock fails CI on arrival, which is what keeps the reachability predicate
   total after P9.

   > **DEBT-3 — restated by the #580 PR; the inventory is closed, the interlock is still owed.**
   > The rule used to say "the exact nine", and both the count and the rule's NAME-only shape were
   > part of the gap. Corrected: `prepare_to_wait_checked` is `pub(crate)`, and the census filtered
   > prefix-matched definitions on a literal `pub` token — the code byte before `fn` in
   > `pub(crate) fn` is `)` — so a genuine tenth family member was invisible to the ratchet whose
   > entire job was to forbid one. The filter is dropped. Of the four publications this note used to
   > list, `syscall/futex.rs:115` was closed by `16d1a062` (PR #604) and is withdrawn; the direct
   > `BlockedOnIO` write and `kthread_park()` are now inside the family; and the "dead
   > `Thread::set_blocked()`/`Scheduler::block_current` pair" **was not dead** — that claim was false
   > when written, `block_current` had 14 production callers, and the real defect was two stale
   > `#[allow(dead_code)]` attributes, now removed with only the out-of-family mutator deleted.
   > The rule no longer rests on names alone: `tests/teardown_structure.rs` also censuses every
   > `ThreadState::Blocked*` publication, requires each production one to sit in a family-named item,
   > and censuses opaque `state` field stores so the value cannot be laundered through a binding.
   > **Still owed by P9:** the interlock itself. See the DESIGN-DEBT REGISTER.
...
**Liveness-lookup census (gate extra (c)'s real surface): 53 raw sites in `manager.rs`, not 32 —
corrected by P6a PR-1.** The recorded 32 was a `grep -c` line-count artifact (rustfmt wraps the
receiver after `self` at 20 sites) and the spec's own correction to 52 still missed
`reparent_children_to_init`'s `get_mut(child)`, whose key carries no literal `&`. All 53 migrated
onto the tombstone predicates; the census in `tests/teardown_structure.rs` is now **empty** and walks
the same token shape the figure was derived from. The paragraph below is the superseded reasoning,
kept because it names the sites: 9
...
(f) `TOMBSTONE_JOIN{retire_second} > 0` — the reap lands first, before grace has elapsed, so the
retire gate is the second writer that performs the removal. Assert the removal happens at retirement,
not at the reap, and that `waitpid` had already returned the correct status.
(g) `TOMBSTONE_JOIN{reap_second} > 0` — retirement completes first and the reap is the second writer.
Assert the row is removed at the reap, exactly once, with the correct status still returned.
> **P6a PR-2's driver for (b), (f) and (g), and why it is not the workload pair above.** The natural
> 64-child cohort reaches only one arm, and a sleep-based delayed-reap injection would make a merge
> gate depend on a timing race — which this campaign does not grant tolerance for. Both arms are
> driven **deterministically, in one run, on both arches** by `tombstone_join_oracle_test`, a new
> `boot_tests` oracle on rows that genuinely exited: arm 1 reaps then retires, arm 2 retires then
> reaps, and the oracle observes `TOMBSTONE_RESIDENT` **nonzero between the two events** and back to
> its entry value afterwards — both halves of (b)/(f)'s anti-vacuity burden. It is a **new** oracle,
> not a converted one, so none of the eight pinned oracle literals moves. The production census line
> `[TOMBSTONE_CENSUS:…]` carries the same four counters from a real workload, emitted at boot on both
> arches, by the strand oracle's periodic report, and by the heartbeat's `/proc/trace/counters` read.
...
> **P6a PR-2, condition C3.** The repeat-reap leg is the *concurrent* one: two waiters both pass the
> scan, and the reap arm's claim under PM is the arbiter — the loser returns `ECHILD` and copies no
> status. The oracle asserts a second claim on the same row is refused, which is exactly what a
> concurrent losing waiter observes.
(i) A row whose `Resources` is `Absent` is removed at reap with no retirement event at all — the
degenerate arm.
> **P6a PR-2, condition C2: (i) is re-arched to aarch64.** x86_64 always defers — its exit path
> builds a receipt unconditionally — so the synchronous-release arm exists only on aarch64, where
> `defer_live_process_resources` returns `None` for a root that is live in no TTBR0. Running (i) on
> x86 asserts against a workload that architecture cannot produce.
> **Gate extra (d) is declared vacuous by construction, not reported as evidence.**
> `allocate_ordinary_pid` is a bare monotonic `fetch_add`: **PID reuse does not exist on this tree**,
> so DESIGN's *"PID reuse is delayed until the tombstone is removed"* describes a mechanism the
> kernel does not have. (d) cannot fail, and a leg that cannot fail is declared, never counted.
...
> site can forget it and no future wait site can be added without it. **The entry points are TEN,
> not the nine this table used to list** — `prepare_to_wait_checked` is `pub(crate)` and the census
> filtered on a literal `pub` token, so it was invisible to the very ratchet meant to forbid a tenth
> member (DEBT-3, discharged by the #580 PR). The set was complete at `985881a6` only as far as the
> ratchet could see; it is complete and pinned by census shape at `102317b4`. **No line anchors:**
> every one this table used to carry had drifted, which is exactly why the pin is a census.
>
> | Primitive | file |
> |---|---|
> | `Scheduler::block_current` | `task/scheduler.rs` |
> | `Scheduler::block_current_for_signal` | `task/scheduler.rs` |
> | `Scheduler::block_current_for_signal_with_context` | `task/scheduler.rs` |
> | `Scheduler::block_current_for_child_exit` | `task/scheduler.rs` |
> | `Scheduler::block_current_for_timer` | `task/scheduler.rs` |
> | `Scheduler::block_current_for_io` | `task/scheduler.rs` |
> | `Scheduler::block_current_for_io_with_timeout` | `task/scheduler.rs` |
> | `Scheduler::block_current_for_compositor` | `task/scheduler.rs` |
> | `WaitQueueHead::prepare_to_wait` | `task/waitqueue.rs` |
> | `WaitQueueHead::prepare_to_wait_checked` | `task/waitqueue.rs` |
>
> A private eleventh member, `Scheduler::block_current_for_io_publish`, holds the sole `BlockedOnIO`
> write and is reached only from `block_current_for_io_with_timeout`; it is a census row, not an
> entry point, and the interlock installs at its caller.
>
...
2. **All TEN primitives are covered**, not just the four with a syscall entry point: a unit-level
   injection calls each primitive with a latched request and asserts refusal, including
   `block_current_for_compositor`, `block_current_for_io_with_timeout` and
   `prepare_to_wait_checked`, which no userspace test reaches directly. `kthread_park()` and
   `WaitQueueHead::prepare_to_wait` are covered *through* the primitives they now call, not
   separately.
3. **Ratchet:** the blocking-primitive set is exactly the ten above (eleven census rows with the
   private publisher); adding an eleventh entry point without the interlock fails CI.
   **DEBT-3's inventory half is closed — by the #580 PR, which is this gate's precondition, not part
   of it.** Every `ThreadState::Blocked*` publication under `kernel/src` outside a `#[cfg(test)]`
   fixture now sits inside a family-named primitive, so the interlock installs in one place per
   family; and the ratchet no longer pins names alone (census of publications + derived family rule
   + census of opaque `state` stores), so a direct state write cannot re-open the hole while this
   gate reads green. **What this phase still owes is the interlock itself** — the DEBT-3 bullet
   "bring each path under the no-new-block interlock or prove it unreachable once a request is
   latched" is carried here as a P9 acceptance item, citing the #580 PR. Until it ships, DESIGN
   §1.5's one-way-door claim remains an argument P9 owes.
