...
| `ProcessManager::remove_process` at `manager.rs:1101-1104` | **`:1342-1350`** (nine lines; re-derived at `174e5227`) |
...
- **Closure D — the blocking-primitive inventory: v3 counted NINE; the closed inventory is TEN
  entry points, still in two files.** *(**DEBT-3, inventory half discharged by the #580 PR.** v3's
  nine were not the whole family: `WaitQueueHead::prepare_to_wait_checked` is `pub(crate)`, and the
  census filtered on a literal `pub` token, so it was invisible to the very ratchet that exists to
  forbid a tenth member. The other three publications v3 listed as outside the set — `futex.rs`,
  the direct `BlockedOnIO` write, and `kthread_park()` — are now inside it; the "dead
  `set_blocked`/`block_current` pair" claim was false when written and is retracted in §0.3.1.
  What P9 still owes is the interlock itself, not the inventory.)* No line anchors are given below:
  every one v3 recorded had drifted by `102317b4`, and the inventory is pinned by census shape in
  `tests/teardown_structure.rs` (`BLOCKING_PRIMITIVES`), not by line. The ten, as enumerated:
  `Scheduler::block_current`, `block_current_for_signal`, `block_current_for_signal_with_context`,
  `block_current_for_child_exit`, `block_current_for_timer`, `block_current_for_io`,
  `block_current_for_io_with_timeout`, `block_current_for_compositor` (all `task/scheduler.rs`),
  and `WaitQueueHead::prepare_to_wait` + `WaitQueueHead::prepare_to_wait_checked`
  (`task/waitqueue.rs`, with `finish_wait` alongside). An **eleventh census row**,
  `Scheduler::block_current_for_io_publish`, is the private publisher the #580 PR renamed into the
  family; it is not an entry point — only `block_current_for_io_with_timeout` reaches it — but it
  is where the sole `BlockedOnIO` write now lives, which is what lets the census forbid a direct
  state write with no per-site exemption.
...
- `waitpid`'s reap physically removes the row: `syscall/wait.rs:386` calls
  `manager.remove_process(child_pid)` → `manager.rs:1102-1104` *(now `:1342-1350`; §0.3.1)*. This is
...
|---|---|---|
| x86 reap removes the row directly (DEBT-4) | `syscall/handlers.rs:3101` | **`:3128`** (inside `complete_wait` at `:3064`), plus a byte-similar duplicate at `syscall/wait.rs:386` |
| `ProcessManager::remove_process` | `manager.rs:1101-1104` → `:1102-1104` | **`:1342-1350`** — a **nine-line** choke point that already bumps `ROW_REMOVAL_EPOCH` via `note_process_row_removed()` (`task/process_task.rs:355-356`) and clears `designated_init` (P5a) |
...
| Blocking publications outside the nine (DEBT-3) | `futex.rs:115`, `scheduler.rs:2175-2194` | **all four re-verified at `102317b4` and dispositioned by the #580 PR.** `futex.rs:115` was **already closed** by `16d1a062` (PR #604) — citation withdrawn. The `BlockedOnIO` write (cited `:2607`, then `:3049`) is now inside `Scheduler::block_current_for_io_publish`, reached only via `block_current_for_io_with_timeout`. `kthread_park()` calls `Scheduler::block_current()`. The **"dead pair" claim was false when filed** — `block_current` had 14 production callers then and now; the real defect was two stale `#[allow(dead_code)]` attributes, both removed, with `Thread::set_blocked` (the out-of-family mutator) deleted and `block_current` kept. A fifth hole found while verifying: the census's `pub(crate)` blind spot hid `prepare_to_wait_checked`, so the inventory was **ten**, not nine. Tracked as **#580** |
...
>    a refusal the caller maps to `EINTR` (or its family's existing cancel path). **The entry
>    points are TEN, not the nine recorded here at `985881a6`** (DEBT-3, discharged by the #580 PR):
>    `Scheduler::block_current`, `block_current_for_signal`, `block_current_for_signal_with_context`,
>    `block_current_for_child_exit`, `block_current_for_timer`, `block_current_for_io`,
>    `block_current_for_io_with_timeout`, `block_current_for_compositor`,
>    `WaitQueueHead::prepare_to_wait`, and `WaitQueueHead::prepare_to_wait_checked` — the last of
>    which is `pub(crate)` and was invisible to the census that pins the set. Line anchors are
>    deliberately omitted: every one this paragraph used to carry had drifted, and the set is pinned
>    by census shape in `tests/teardown_structure.rs`. The check lives **inside the primitives**, not
>    at their callers, so no wait site can forget it and no future wait site can be added without it
>    (a P0 ratchet rule asserts the primitive set is exactly these ten, plus the private in-family
>    publisher `block_current_for_io_publish` as an eleventh census row). Counted `EXIT_BLOCK_REFUSED{family}`, which P9's own test
...
>
>    ⚠ **DEBT-3 — the inventory half is closed by the #580 PR; the interlock half is still P9's.**
>    The inventory is **ten** entry points, not nine: `prepare_to_wait_checked` is `pub(crate)` and
>    the census filtered on a literal `pub` token, so it was hidden from the ratchet. Of the four
>    publications this warning used to list, `syscall/futex.rs:115` was closed by `16d1a062`
>    (PR #604) and its citation is withdrawn; the direct `BlockedOnIO` write now lives inside
>    `Scheduler::block_current_for_io_publish`; `kthread_park()` calls `Scheduler::block_current()`;
>    and the "dead `Thread::set_blocked`/`Scheduler::block_current` pair" **was not dead** — that
>    claim was wrong when it was written (14 production callers then and now), the real defect was
>    two stale `#[allow(dead_code)]` attributes, and both are gone with only the out-of-family
>    mutator deleted. The ratchet no longer pins names alone: a census of blocked-state
>    publications, a derived rule that every production publication sit in a family-named item, and
>    a census of opaque `state` field stores now catch a direct write anywhere under `kernel/src`.
>    **What remains owed is the interlock itself** — the pre-block acquire-load and refusal — and
>    therefore the one-way-door claim below. It is a P9 acceptance item whose precondition, one
>    installation point per family, the #580 PR has now met. See the DESIGN-DEBT REGISTER.
>
...
`remove_process`, `manager.rs:1342-1350`), and the `Resources` obligation **by construction outlives
