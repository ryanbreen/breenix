...
**Status:** **Tranche-ratified document. Tranche 1 (P0+P1+P2): RATIFIED and COMPLETE — merged to `main`. Tranche 2 (P3 incl. the folded creation-path parity, + P5a; P4 dissolved, P5b held on #575): RE-RATIFIED by the operator 2026-08-16 against `P3-RERATIFICATION-2026-08-15.md` — see PLAN §0.0. Later phases: design-debt register applies; sections may change before their tranche ratifies. Tranche-2 anchors re-verified at `main` @ `2c7b8798` — §0.3.1 governs where earlier sections disagree.**
...
> own none of the debts below** (their owners are P6a, P6b, P7,
...
| `ProcessState::Creating` window (P3) | — | `process/process.rs:54`, set at `:318`, cleared to `Ready` by `set_main_thread` at **`:350`** |
| PM→SCHEDULER nesting on creation paths (was P4, now P3 part 3) | "the spawn/test-disk paths" | **`creation.rs:67→85`**, **`creation.rs:185→202`**, **`boot/test_disk.rs:258→263`**; `scheduler::spawn` at `task/scheduler.rs:3444` takes `lock_scheduler()` at `:3447` |
| Kernel-stack ownership (was P4/AC-8) | "apply the existing `take()` to three more sites" | **impossible as written** — `manager.rs:851`, `:925`, `:1010` `Box::leak` the `KernelStack` at construction with `kernel_stack_allocation: None`. Tracked as **#579**. The correct-transfer contrast sites are `syscall/clone.rs:250-252`, `arch_impl/aarch64/syscall_entry.rs:961`, `manager.rs:1833` |
| Freed-row stack hazard (was P4's premise) | "freed ungated at `waitpid` reap" | **structurally live, merely unreached**: `remove_process` drops the row → `Process::main_thread` (`process/process.rs:198`) → `Thread::kernel_stack_allocation` (`task/thread.rs:428`) → `impl Drop for KernelStack` (`memory/kernel_stack.rs:85-99`); nothing ever clears `main_thread`. Unreached only because of the #579 leak |
...
| **8** | Sibling kernel stacks freed ONLY behind two-epoch grace via scheduler ownership | **NOT closed by this plan — discharged on #579.** The transfer this AC specifies cannot be applied to the spawn/direct-init/test-disk paths: all three `create_main_thread*` constructors `Box::leak` the `KernelStack` at construction (`manager.rs:851`, `:925`, `:1010`) and store `kernel_stack_allocation: None`, so no allocation object survives to hand to the scheduler copy. Fork (`manager.rs:1833`) and CLONE_VM clone (`syscall/clone.rs:250-252`, `arch_impl/aarch64/syscall_entry.rs:961`) already transfer correctly. The freed-row hazard is **structurally live and merely unreached** — `remove_process` drops the row and its `main_thread`, whose `KernelStack` `Drop` returns the slot, and nothing ever clears `main_thread`; it is unreached only because the leak means no row holds one. **#546** is the *user*-stack sibling and does not substitute. | ~~4~~ **#579** | Ownership assertion after every creation path (exactly one owner, and it is the scheduler copy); 1000-iteration fork/clone/spawn exit stress with stack-pool accounting (allocated == freed) and an allocator assertion that never selects a live slot |
