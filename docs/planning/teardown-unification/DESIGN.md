...
| PM→SCHEDULER nesting on creation paths (**P4**, PR 2) | "the spawn/test-disk paths" | **`creation.rs:67→85`**, **`creation.rs:185→202`**, **`boot/test_disk.rs:258→263`**; `scheduler::spawn` at `task/scheduler.rs:3444` takes `lock_scheduler()` at `:3447` |
| Kernel-stack leak surface (**P4**/AC-8, PR 1) | "apply the existing `take()` to three more sites" | **impossible as written, and the surface is FIVE sites, not three.** `grep -n 'Box::leak' kernel/src/process/manager.rs` returns `851`, `925`, `1010` (the three `create_main_thread*` constructors) **plus `1979`** (`complete_fork`, fn `:1920`, `#[cfg(target_arch = "x86_64")]`) **and `2323`** (`fork_process_with_context`, fn `:2148`, `#[cfg(target_arch = "x86_64")]`, `kernel_stack_allocation: None` at `:2339`) — every one leaks the `KernelStack` at construction with an in-tree `// TODO: proper cleanup`. Tracked as **#579**, which **P4 closes** |
...
| Freed-row stack hazard (**P4**'s premise) | "freed ungated at `waitpid` reap" | **structurally live, merely unreached**: `remove_process` drops the row → `Process::main_thread` (`process/process.rs:199`) → `Thread::kernel_stack_allocation` (`task/thread.rs:428`) → `impl Drop for KernelStack` (`memory/kernel_stack.rs:85-99`); nothing ever clears `main_thread`. Unreached only because of the five-site leak — which is why P4's PR 1 fixes the leak and ships AC-8's accounting gate together |
| `complete_fork` suppression (adjacent to P4) | — | `manager.rs:1919` carries `#[allow(dead_code)]` while the function has **two live callers** (`:1490`, `:1634`) — a stale suppression against the repo's zero-tolerance standard; removed by P4's PR 1 |
...
rule-5 reconciliation).** v1 claimed "13 phases / 13 PRs", which the first ratification correctly
called a contradiction with the split phases; v2 said 16 PRs; v3 said 17. The honest figure today is
**13 numbered phases / 22 PRs** (one of them, P5b, held): P6 splits into P6a + P6b (condition 2), P10
into P10a + P10b + P10c + P10d (four wait families, the last of which also deletes the legacy arm),
P5 into P5a + P5b, and **P3, P4, P5a and P9 are each two PRs because each already carries two
independently revertable mechanisms**. **There is no size rule of any kind.** The ~230-line / 5-file
