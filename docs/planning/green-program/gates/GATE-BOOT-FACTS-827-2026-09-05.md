...
| (empty) | QEMU still alive at the kill point | `poll_exhausted` |
| (empty) | QEMU already dead at the kill point | `hard_timeout` -- the `timeout 20` wrapping the launch fired first |
...
| neither | QEMU still alive at the kill point | `poll_exhausted` |
| neither | QEMU already dead at the kill point | `hard_timeout` |
...
exit: 101 (test binary FAILED; "test result: FAILED. 0 passed; 1 failed")
assertion: "docker/qemu/run-aarch64-boot-test-strict.sh never sets ended_by
            to: [\"poll_exhausted\"] -- every one of #827's 5 ended_by
            values must be reachable from this gate's own poll-loop control
            flow"
```
...
```
cargo test -p breenix <--test N> x32 (31 pre-existing + this branch's own
  tests/gate_boot_facts_structure.rs)
-> 33/33 green, 591 test cases total (5 of them this branch's own)
