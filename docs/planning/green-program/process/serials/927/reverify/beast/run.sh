...
exec > "$TMPDIR/reverify/driver.log" 2>&1
...
for item in 'boot-only|bash docker/qemu/run-x86-boot-tests-only.sh' 'full|bash docker/qemu/run-x86-boot-tests.sh' 'oracle|bash docker/qemu/run-blocking-io-oracle-gate.sh --arch x86_64' 'precommit|cargo build --release --features testing,external_test_bins --bin qemu-uefi' 'prod|bash docker/qemu/run-x86-prod-profile-boot-test.sh'; do
