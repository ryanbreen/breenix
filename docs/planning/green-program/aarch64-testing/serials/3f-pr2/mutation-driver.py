...
p=Path('kernel/src/task/scheduler.rs');original=p.read_text();root=Path.cwd();ev=root/'docs/planning/green-program/aarch64-testing/serials/3f-pr2';env=os.environ.copy();env['TMPDIR']=str(root/'.tmp');env['BREENIX_GATE_TMP']=str(root/'.gate-tmp')
