...
| `testing-profile-boot-at-06d149b6.txt` | The same profile at the round-1 branch tip, against the regenerated fixture. `[test] Loaded 78/78` at line 1900 -- the round-1 doc's number is reproducible once the fixture matches the catalog. The round-1 review read a lower count from an ELF set that predated the catalog's 78th entry; that count is the review's own and appears in no committed serial here. |
