...
| file | what it is |
|---|---|
| `01-strict-boot1-serial.txt` | re-recorded five times, each time by a landing whose branch changed a required line one of the two replay tests' gate scorers checks: `#812` (added `IRQ_HOLD_ORACLE`), `#819`'s first merge (rewrote `FCNTL_PM_CONTENTION_ORACLE` to the rendezvous shape), `#627` (added `arm_delay_us` to `FUTEX_HANDOFF_ORACLE`, independently and concurrently with `#819`), `#819`'s second merge (recombined all three requirements after `#627` landed first, `BUILD_ID 006a9c7cb301c7`), and failure-trace-capture PR-2 (added the `RING_SPAN` ring-depth self-check requirement, `BUILD_ID 006a9c88023938`) |
