...
- **Bound and re-kick:** `run_all_tests()` captures one CNTVCT anchor immediately before entering the EarlyBoot stage runner and passes that same value through both EarlyBoot and PostScheduler. All subsystem joins across those two phases therefore share one combined 60-second deadline; EarlyBoot duration cannot re-arm PostScheduler's budget, and sequential handles cannot multiply it. While a handle remains unexited, the coordinator broadcasts `SGI_RESCHEDULE` about every 50ms to each actual online CPU found by scanning `0..smp::MAX_CPUS` with `is_cpu_online()`. There is deliberately no partial-progress re-arm: ordinary subsystem threads expose no honest worker-progress signal. A zero `CNTFRQ_EL0` fails immediately, and an unchanged CNTVCT sample across 100,000 coordinator iterations emits a distinct watchdog failure.
- **Diagnostics:** Deadline expiry prints one `[boot_tests] subsystem_join_timeout` evidence line containing subsystem, stage, per-handle elapsed milliseconds, elapsed milliseconds from the shared boot-test join anchor, re-kick count, and online CPU count. It also emits `[BOOT_TESTS:FAIL:subsystem_join_timeout]`, marks the subsystem failed, and stops joining further handles, so the external harness receives an attributed failure instead of waiting for its outer kill.
- **Harness relationship:** The 60-second combined ceiling leaves 30 seconds before the harness's 90-second Phase-1 poll and 60 seconds before its 120-second QEMU kill for pre-anchor boot work and post-expiry scheduling. Unlike the former per-stage 80-second clocks, this bound never becomes `EarlyBoot elapsed + 60s`. The in-guest marker is guaranteed ahead of the 90-second poll when the anchor is reached within the first 30 seconds and QEMU receives host time after expiry. Absolute assurance under an arbitrarily long interval in which the host never schedules QEMU is impossible for any in-guest watchdog; that remaining exposure is specifically a greater-than-30-second pre-anchor delay or no QEMU scheduling between the 60-second expiry and the outer kill, not stacked stage budgets.
