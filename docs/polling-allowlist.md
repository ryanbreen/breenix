...
- **Progress bound:** Each wait fails after three consecutive CNTVCT seconds without relevant work. Publish-phase waits re-arm on their existing reservation/readiness/publish-attempt counters. Joins re-arm on advance of either that work counter or a lock-free per-TID exit-stage counter, whose writes occur only while the awaited kthread executes its teardown tail. The target remains in the test-only affinity-clear stage until its join captures the counter baseline and arms that exact TID, guaranteeing a post-baseline teardown step instead of another terminal-before-wait signal. CPU timer ticks are not a progress source: a ticking CPU that never runs the pinned kthread therefore still fails in about three seconds. If CNTFRQ_EL0 reports zero, this watchdog fails immediately with a named counter-unavailable verdict instead of guessing a frequency that could either shrink or stretch the liveness window.
- **Absolute bound:** Every wait also has a distinct 30-second absolute ceiling and returns immediately on failure. There is no aggregate gate deadline; at most eight normal-path waits bound total gate time by construction. A separate detector samples CNTVCT every 10,000,000 coordinator iterations and fails even if the awaited condition becomes true after the counter-stall observation.
- **Evidence:** Failures report starting/final work and exit-stage values, whether either source advanced, milliseconds since the last advance, and the most recent five chronological one-second progress samples, alongside re-kick SGIs and online CPU count. Any wait that re-arms and runs beyond three seconds emits one compact breadcrumb per second with its name, elapsed time, current work/exit-stage values, and re-kick count.
