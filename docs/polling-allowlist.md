...
- **Loop:** `while kernel::arch_impl::aarch64::smp::cpus_online() < expected { ... core::hint::spin_loop(); }` observes both the online count and the sum of monotonic per-CPU bring-up stages. An advance of either re-arms the no-progress window. Once per second, each still-offline CPU gets a stage-number/name breadcrumb; final diagnostics add its last PSCI status and whether its stage advanced during this wait.
...
- **Progress bound:** Each wait has an unconditional eight-second floor from its own entry plus a three-second no-further-progress deadline from the most recent observed advance. The effective no-progress deadline is `max(wait_start + 8s, last_advance + 3s)`, so one early advance in a multi-worker union cannot shorten an unseen sibling's first-schedule allowance. Only worker-owned work/publish counters or lock-free per-TID exit-stage counters contribute; CPU timer ticks and other CPU-level activity do not. The storm waits use a saturating union of both publishers' and the observer's counters, so that signal is dependency-wide rather than target-attributable, and the observer counter advances on its polling iterations. The exit-stage terminal increment occurs after the kthread's `exited` store; the first exit-stage increment occurs before affinity is cleared.
- **Hard bounds and recovery:** Every wait has a distinct 15-second absolute ceiling, kicks every CPU in its dependency set, and resends the reschedule SGI about every 50ms. Every 100,000 coordinator iterations, an unconditional delta check fails if CNTVCT made zero progress. If `CNTFRQ_EL0` is zero, the gate fails immediately instead of guessing a frequency. The condition is re-read at a verdict; a late-true condition logs evidence and returns success for that wait. The next wait receives fresh floor/window/ceiling clocks.
- **Diagnostics:** At a verdict, one line records wait/cause, elapsed time, effective deadline budget, re-kick count, online CPUs, current condition, progress start/final values, and time since last progress. For a no-progress verdict, `window_budget_ms` is the effective deadline measured from wait entry (at least eight seconds and possibly later after progress); it is 15 seconds for ceiling expiry and zero for counter/join failures. Every wait lasting at least three seconds emits at most one breadcrumb per second.
- **Aggregate boundedness:** There is no shared gate deadline. Each individual wait is bounded and every verdict or late-true recovery is evidenced, but multiple long waits can accumulate and are still subject to the boot harness's separate Phase-1 timeout.
...
- **Bound and re-kick:** All subsystem joins in one stage share one 80-second CNTVCT deadline measured from the start of the join phase. Because all workers were spawned before joining starts, the 11 sequential EarlyBoot handles cannot multiply this ceiling. While a handle remains unexited, the coordinator broadcasts `SGI_RESCHEDULE` about every 50ms to every CPU in `0..cpus_online()`. There is deliberately no partial-progress re-arm: ordinary subsystem threads expose no honest worker-progress signal. A zero `CNTFRQ_EL0` fails the watchdog immediately rather than inventing a frequency.
- **Diagnostics:** Deadline expiry prints one `[boot_tests] subsystem_join_timeout` evidence line containing subsystem, stage, per-handle elapsed milliseconds, shared stage elapsed milliseconds, re-kick count, and online CPU count. It also emits `[BOOT_TESTS:FAIL:subsystem_join_timeout]`, marks the subsystem failed, and stops joining further handles, so the external harness receives an attributed failure instead of waiting for its outer kill.
- **Budget caveat:** The shared 80-second ceiling is 10 seconds below the harness's 90-second Phase-1 budget and 40 seconds below its 120-second QEMU kill, but Phase 1 starts before the join phase, so the 10 seconds is not guaranteed headroom. The recorded `fail_002` starvation run remained silent beyond 80 seconds, so this bound closes the unbounded diagnostic gap but cannot promise recovery under that extreme 14-host-hog profile; accommodating a longer stall requires changing the harness budget or stress level.
