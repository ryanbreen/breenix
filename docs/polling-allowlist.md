...
- **Bounded:** In `boot_tests` builds, twenty seconds of CNTVCT time without an online-count or bring-up-stage advance ends the wait; real secondary progress re-arms that window, and a separate forty-second ceiling runs from the wait's start. That local ceiling is also capped by the same 65-second liveness budget used by P20, anchored near kernel entry before initialization, SMP release, or boot tests. Consequently P17 and P20 together can consume at most 65 seconds, leaving 90s - 65s = 25s of the full-test script's Phase-1 deadline for loader/process overhead, kernel initialization, the other subsystem tests, and marker polling. A pathological full forty-second P17 wait leaves P20 no more than twenty-five seconds because earlier initialization also consumes the shared clock; that is the deliberate last-resort backstop. It does not constrain the realistic proof case: `prove-starved/fail_016` needed roughly eight seconds for its longest wait, and observed starved-run totals are <=10 seconds from the early shared anchor. Plain kernels use a five-second no-progress window and ten-second absolute ceiling so the final offline-CPU diagnostics print well before the strict harness's 20-second kill and the native harness's 30-second kill. An unconditional delta check every 10,000,000 loop iterations ends the wait with a distinct `[smp] CNTVCT stalled` diagnostic if the counter made no advance between samples, so a fully frozen time source cannot make the spin unbounded. A zero `CNTFRQ_EL0` reading uses the lowest plausible 1MHz fallback and therefore errs toward shorter deadlines. A missing shared anchor ends the boot-test wait immediately. The loop exits as soon as all expected CPUs are online.
...
- **Loop:** At most four CPU_ON attempts. Each attempt preserves the existing HVC64 → HVC32 conduit order; only transient `DENIED` and `INTERNAL_FAILURE` statuses are retried. Permanent `INVALID_PARAMS` and `NOT_PRESENT` statuses end the probe immediately. The 500µs inter-attempt backoff is bounded by both CNTVCT and a 1,000,000-iteration safety cap.
- **Justification:** A transient firmware/hypervisor failure should not permanently reduce the expected CPU count. `SUCCESS`, `ALREADY_ON`, and `ON_PENDING` are accepted; `ON_PENDING` is left to P17's separately bounded online wait. Permanent topology/probe errors are returned without retry, and SMC remains excluded.
...
- **Aggregate boundedness:** P17 and P20 share one 65-second ceiling: `min(P17 local 40s + P20 local 45s, shared 65s) = 65s`, leaving `90s - 65s = 25s` for the rest of Phase 1. A gate whose condition remains false at its local ceiling emits `cause=gate_ceiling`; one that exhausts the shared remainder emits `cause=phase_one_ceiling`. Both produce the same actionable `exit_kick_gate: ... unresponsive` site failure.
