...
- **Progress bound:** Each wait has a fresh eight-second first-progress window until that wait observes an advance, then a three-second no-progress window. The active window re-arms only when an awaited worker-owned work/publish counter or lock-free per-TID exit-stage counter advances; CPU timer ticks and other CPU-level activity are not progress. The storm waits use the saturating union of both publishers' and the observer's work counters because each join depends on that chain. The exit-stage terminal increment occurs after the kthread's `exited` store, so a terminal counter cannot precede a false join condition. The first exit-stage increment occurs before affinity is cleared, proving the pinned worker reached its exit tail. A target that never runs fails in eight seconds; one that ran and wedged fails within three seconds of its true stall.
- **Hard bounds and recovery:** Every wait has a distinct 15-second absolute ceiling, kicks every CPU in its dependency set, and resends the reschedule SGI about every 50ms. Every 100,000 coordinator iterations, an unconditional delta check fails if CNTVCT made zero progress. If `CNTFRQ_EL0` is zero, the gate fails immediately instead of guessing a frequency. The condition is re-read immediately before failure; a recovered late-true wait logs evidence and the next wait receives a fresh window. The eight-second first-progress bound remains well inside the strict harness's 20-second kill and the 90-second Phase-1 budget.
- **Diagnostics:** Before failure, one line records wait/cause, elapsed and the governing window milliseconds, re-kick count, online CPUs, current condition, progress start/final values, and time since last progress. `window_budget_ms` is eight or three seconds for no-progress, 15 seconds for ceiling expiry, and zero for counter/join failures. Every wait lasting at least three seconds emits at most one breadcrumb per second, including waits that have not yet observed first progress.
- **Aggregate boundedness:** There is no shared gate deadline. A ceiling expiry logs evidence, fails, and returns from the gate immediately. Multiple long waits can therefore accumulate only through repeated late-true recovery, with each recovery logged; observed clean totals are milliseconds and starved totals are single-digit seconds, so the gate remains well inside the 90-second Phase-1 window. Intervening coordinator work is finite.
