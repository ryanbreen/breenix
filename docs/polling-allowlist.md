...
- **Bounded:** Twenty seconds of CNTVCT time without an online-count or bring-up-stage advance ends the wait; real secondary progress re-arms that window. A separate forty-second CNTVCT ceiling runs from the wait's own start regardless of progress. The locked starvation protocol runs only the `boot_tests` build under `run-aarch64-full-test.sh --boot-tests-only` (or an equivalent 120-second proof gate), not the strict harness's plain build; forty seconds leaves fifty seconds for the remaining boot tests before the tracked full-test script's tighter 90-second Phase-1 deadline and eighty seconds before its 120-second QEMU kill. An unconditional delta check every 10,000,000 loop iterations ends the wait with a distinct `[smp] CNTVCT stalled` diagnostic if the counter made no advance between samples, so a fully frozen time source cannot make the spin unbounded. A zero `CNTFRQ_EL0` reading uses the lowest plausible 1MHz fallback and therefore errs toward shorter deadlines. The loop exits as soon as all expected CPUs are online.
...
- **Loop:** At most four CPU_ON attempts. Each attempt preserves the existing HVC64 → HVC32 conduit order; any status not accepted as `SUCCESS`, `ALREADY_ON`, or `ON_PENDING` is retried. The 500µs inter-attempt backoff is bounded by both CNTVCT and a 1,000,000-iteration safety cap.
...
- **Progress bound:** Each wait has an unconditional eight-second floor from its own entry plus a three-second no-further-progress deadline from the most recent observed advance. The effective no-progress deadline is `min(max(wait_start + 8s, last_advance + 3s), gate_start + 45s)`, so one early advance in a multi-worker union cannot shorten an unseen sibling's first-schedule allowance and no recovery can extend the whole gate. Only worker-owned work/publish counters or lock-free per-TID exit-stage counters contribute; CPU timer ticks and other CPU-level activity do not. The dependency-wide saturating union of both publishers' and the observer's counters is used only while waiting for `workers_ready == 3`. Each storm join instead uses the awaited worker's own counter plus its per-TID exit stages, so an observer polling loop cannot re-arm a publisher's no-progress window. The exit-stage terminal increment occurs after the kthread's `exited` store; the first exit-stage increment occurs before affinity is cleared.
- **Hard bounds and recovery:** Every wait has a distinct 15-second absolute ceiling, and the entire gate invocation has a separate 45-second ceiling measured once before gate work begins. Each wait checks both bounds, kicks every CPU in its dependency set, and resends the reschedule SGI about every 50ms. Every 100,000 coordinator iterations, an unconditional delta check fails if CNTVCT made zero progress. If `CNTFRQ_EL0` is zero, the gate fails immediately instead of guessing a frequency. The condition is re-read at a verdict; only a no-progress verdict that raced a late-true condition returns success. Absolute/gate ceilings and counter failures remain failures, and the shared gate clock is never re-armed.
- **Diagnostics:** At a verdict, one line records wait/cause, elapsed time, effective deadline budget, re-kick count, online CPUs, current condition, progress start/final values, and time since last progress. For a no-progress verdict, `window_budget_ms` is the effective deadline measured from wait entry (at least eight seconds and possibly later after progress); it is 15 seconds for per-wait ceiling expiry, 45 seconds for aggregate gate expiry, and zero for counter/join failures. Only liveness deadline failures retain the CPU-specific `exit_kick_gate: ... unresponsive` permanent message. Counter-frequency, CNTVCT-stall, exit-progress, and final-join failures use cause-specific permanent messages and therefore cannot be misclassified by the canonical unresponsive grep. Every wait lasting at least three seconds emits at most one breadcrumb per second.
- **Aggregate boundedness:** The 45-second shared ceiling prevents multiple 15-second waits or late-true recoveries from accumulating to the harness's 90-second Phase-1 timeout. A gate whose condition remains false at that ceiling emits `cause=gate_ceiling` and an actionable `exit_kick_gate: ... unresponsive` failure.
...
## P21: aarch64 boot-test subsystem kthread joins

- **File:** `kernel/src/test_framework/executor.rs` (`join_boot_test_kthread_bounded()` and the `run_staged_tests()` join phase)
- **Loop:** On `aarch64` builds with `boot_tests`, the synchronous stage coordinator polls each subsystem kthread's monotonic `exited` flag instead of entering the general-purpose unbounded `kthread_join()` wait. Production kernels and x86_64 builds retain the plain join path unchanged.
- **Bound and gate nesting:** `run_all_tests()` captures one CNTVCT anchor immediately before entering the EarlyBoot stage runner and passes that same value through both EarlyBoot and PostScheduler. Ordinary subsystem joins across those phases share one combined 60-second deadline; EarlyBoot duration cannot re-arm PostScheduler's ordinary budget, and sequential handles cannot multiply it. The Process/PostScheduler worker is deliberately different because it contains the exit-kick gate. The gate publishes the exact CNTVCT value used for its own 45-second aggregate ceiling. Once published, the executor measures that Process join from the same timestamp and permits 60 seconds: the gate's full 45 seconds plus a fixed 15-second margin (one complete gate per-wait ceiling) for returning the `TestResult`, emitting the subsystem result, and reaching the kthread `exited` store under host-vCPU starvation. Thus the outer executor watchdog is strictly nested outside the inner gate watchdog and cannot replace an in-budget, CPU-attributed gate verdict with a generic join timeout. Before the gate timestamp is published, the shared 60-second deadline still bounds a Process worker that never reaches the gate.
- **Re-kick and counter-stall detection:** While a handle remains unexited, the coordinator sends `SGI_RESCHEDULE` about every 50ms to each actual online CPU except the coordinator's current CPU, found by scanning `0..smp::MAX_CPUS` with `is_cpu_online()`. There is deliberately no partial-progress re-arm: ordinary subsystem threads expose no honest worker-progress signal. The loop calls `yield_current()` and `spin_loop()` rather than WFI, so the unchanged-CNTVCT check is actually reached after 100,000 coordinator iterations even when the counter-backed virtual timer cannot wake WFI. Timer preemption remains enabled, so the coordinator does not monopolize progress while it samples. A zero `CNTFRQ_EL0` fails immediately.
- **Diagnostics:** Deadline expiry prints one `[boot_tests] subsystem_join_timeout` evidence line containing subsystem, stage, the selected deadline scope and budget, per-handle elapsed milliseconds, elapsed milliseconds from the shared boot-test join anchor, re-kick count, and online CPU count. It also emits `[BOOT_TESTS:FAIL:subsystem_join_timeout]` and marks the subsystem failed. No safe kthread cancellation primitive exists, so the executor deliberately stops consuming the remaining handles after that terminal marker instead of cascading the same expired deadline across them; those workers remain detached until the external harness ends QEMU.
- **Harness relationship:** Ordinary joins retain the shared 60-second ceiling, leaving 30 seconds before the harness's 90-second Phase-1 poll when the anchor is reached promptly. The Process join's 60-second gate-relative ceiling may extend beyond that ordinary deadline, by design: any exit-kick run that completes within the harness's own Phase-1 window can no longer be failed earlier by the executor, while an actually stuck gate still emits its richer evidence at its 45-second ceiling, fifteen seconds before the executor may intervene. If QEMU receives no host CPU time long enough for the in-guest gate verdict itself to miss the external poll, no in-guest watchdog can guarantee an earlier marker; the executor no longer creates a competing shorter deadline in that case.
- **Status:** ALLOWLISTED — aarch64 `boot_tests`-only bounded coordinator wait.
