...
generate_gdb_script() {
  local ahci_irq ahci_isr_count ahci_last_cpu polled polled_post polled_pre polled_sched timer_ticks timer_hw_ticks timer_total
  ...
  polled_pre="$(sym_addr '^ahci_polled_post_reg_pre_scheduler$')"
  polled_sched="$(sym_addr '^ahci_polled_post_reg_scheduler_running$')"
...

  {
    ...
    echo "ahci_polled_post_reg_pre_scheduler_addr=$polled_pre"
    echo "ahci_polled_post_reg_scheduler_running_addr=$polled_sched"
...
echo === TURN7 AHCI POLLING ATTRIBUTION ENDPOINT STATE ===\\n
...
set \$AHCI_POLLED_POST_REG_PRE_SCHEDULER = $polled_pre
set \$AHCI_POLLED_POST_REG_SCHEDULER_RUNNING = $polled_sched
...
printf "ahci_polled_post_reg_pre_scheduler=%lu\\n", *(unsigned long*)\$AHCI_POLLED_POST_REG_PRE_SCHEDULER
printf "ahci_polled_post_reg_scheduler_running=%lu\\n", *(unsigned long*)\$AHCI_POLLED_POST_REG_SCHEDULER_RUNNING
...
write_summaries() {
  ...
  awk -F= '/ahci_polled_post_registration_count=/ { print $2 }' "$ART/gdb-endpoint-state.log" 2>/dev/null | tail -1 >"$ART/post-registration-polling-counter.txt"
  awk -F= '/ahci_polled_post_reg_pre_scheduler=/ { print $2 }' "$ART/gdb-endpoint-state.log" 2>/dev/null | tail -1 >"$ART/pre-scheduler-polling-counter.txt"
  awk -F= '/ahci_polled_post_reg_scheduler_running=/ { print $2 }' "$ART/gdb-endpoint-state.log" 2>/dev/null | tail -1 >"$ART/scheduler-running-polling-counter.txt"
  if [ ! -s "$ART/polling-counter.txt" ]; then
    echo "unavailable" >"$ART/polling-counter.txt"
  fi
  if [ ! -s "$ART/post-registration-polling-counter.txt" ]; then
    echo "unavailable" >"$ART/post-registration-polling-counter.txt"
  fi
  if [ ! -s "$ART/pre-scheduler-polling-counter.txt" ]; then
    echo "unavailable" >"$ART/pre-scheduler-polling-counter.txt"
  fi
  if [ ! -s "$ART/scheduler-running-polling-counter.txt" ]; then
    echo "unavailable" >"$ART/scheduler-running-polling-counter.txt"
  fi
}
...
log "starting Turn 7 AHCI polling-attribution single Parallels boot"
heartbeat
qemu_cleanup
delete_breenix_vms
rm -f "$ART/gdb-endpoint-state.log" "$ART/polling-counter.txt" "$ART/post-registration-polling-counter.txt" "$ART/pre-scheduler-polling-counter.txt" "$ART/scheduler-running-polling-counter.txt" "$ART/single-boot-serial.log" "$ART/serial-signals.log"
rm -f "$SERIAL_LOG"
...

{
  ...
  echo "post_registration_polling_counter=$(cat "$ART/post-registration-polling-counter.txt" 2>/dev/null || echo unavailable)"
  echo "pre_scheduler_polling_counter=$(cat "$ART/pre-scheduler-polling-counter.txt" 2>/dev/null || echo unavailable)"
  echo "scheduler_running_polling_counter=$(cat "$ART/scheduler-running-polling-counter.txt" 2>/dev/null || echo unavailable)"
  ...
  echo "cpu0_timer_related_markers=$(grep -aciE 'CPU0.*timer|timer regression|CPU0-tick alarm' "$ART/single-boot-serial.log" 2>/dev/null || true)"
...
finish
log "Turn 7 polling-attribution single boot harness complete"
