...
write_result() {
  ...
  serial_bytes="$(wc -c <"$RUN_DIR/run.serial.log" 2>/dev/null | tr -d ' ' || echo 0)"
  window_serial_bytes="$(wc -c <"$RUN_DIR/window.serial.log" 2>/dev/null | tr -d ' ' || echo 0)"
...
log "starting turn7 Parallels BWM softlock capture run=$RUN_NAME max=${MAX_SECONDS}s mode=$MODE"
heartbeat
qemu_cleanup
delete_breenix_vms
: >"$SERIAL_LOG"
