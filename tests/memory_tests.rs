...
fn test_memory_regions() {
    ...
    assert!(
        output.contains("regions") || output.contains("MiB of usable memory"),
        "Memory regions enumeration failed"
    );

    // Check for reasonable memory size (at least 32MB)
    let found_memory = output
        .lines()
    ...
    assert!(
        memory_size >= 32,
        "Insufficient memory detected: {} MiB",
        memory_size
    );
