...
fn test_guard_page_initialization() {
    ...
    assert!(
        output.contains("Initializing stack allocation system..."),
        "Stack allocation system not initialized"
    );

    assert!(
        output.contains("Stack allocation system initialized"),
...
fn test_memory_management_completeness() {
    ...
    assert!(
        output.contains("Initializing frame allocator..."),
    ...
    assert!(
        output.contains("Initializing paging..."),
        "Paging initialization not started"
    );

    assert!(
        output.contains("Page table initialized"),
        "Page table not initialized"
    );

    assert!(
        output.contains("Initializing heap allocator..."),
        "Heap allocator initialization not started"
    );

    assert!(output.contains("Heap initialized"), "Heap not initialized");
