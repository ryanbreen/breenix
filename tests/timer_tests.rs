mod shared_qemu;
use shared_qemu::{extract_timestamps, get_kernel_output};
...
fn test_timer_initialization() {
    ...
    assert!(
        output.contains("Timer initialized"),
...
fn test_delay_functionality() {
    ...
    assert!(
        output.contains("Testing delay macro"),
        "Delay test not found"
    );
    assert!(
        output.contains("Time after delay"),
...
fn test_rtc_functionality() {
    ...
    assert!(
        output.contains("Current Unix timestamp"),
        "RTC timestamp not found"
    );

    // Verify we get a reasonable timestamp (after year 2020)
    if let Some(timestamp_line) = output
        .lines()
        .find(|line| line.contains("Current Unix timestamp"))
    {
