...
fn test_ring3_smoke() {
    ...
    let found_userspace_syscall =
        output.contains("R3-SYSCALL ENTRY") && output.contains("Syscall from Ring 3 confirmed");
    ...
    assert!(
        ...
        "Kernel double faulted during Ring 3 test"
    );
    assert!(
        !output.contains("TRIPLE FAULT"),
        "Kernel triple faulted during Ring 3 test"
    );
    ...
    if found_userspace_breakpoint || found_userspace_syscall || found_userspace_output {
        ...
        assert!(
            no_swapgs_issues,
            "Ring 3 test detected swapgs handling issues"
        );
