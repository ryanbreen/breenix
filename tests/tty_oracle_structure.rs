...
/// distinction. `the_oracle_tally_matches_its_arm_census` above already
/// covers the (now unconditional) ARM_COUNT for both arches; a separate
/// x86-gated ARM_COUNT check would have nothing to read since there is no
/// longer an x86_64-gated declaration to find.
#[test]
