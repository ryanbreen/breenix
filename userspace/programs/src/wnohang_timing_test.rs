...
use libbreenix::process::{self, ForkResult, WNOHANG, yield_now, wifexited, wexitstatus};
...
unsafe fn raw_waitpid(pid: i32, status: *mut i32, options: i32) -> i64 {
    raw::syscall3(libbreenix::syscall::nr::WAIT4, pid as u64, status as u64, options as u64) as i64
}
...
fn main() {
    ...
    match process::fork() {
        ...
        Ok(ForkResult::Parent(child_pid)) => {
            ...
            let wnohang_result = process::waitpid(fork_result, &mut status, WNOHANG);
            ...
            let early_wait_ok = match wnohang_result {
                Ok(pid) if pid.raw() == 0 => {
                    println!("  waitpid(-1, WNOHANG) returned: 0");
                    println!("  PASS: Returned 0 (child still running)");
                    true
                }
                Ok(pid) if pid.raw() as i32 == fork_result => {
                    // Child finished very quickly - this is acceptable
                    println!("  waitpid(-1, WNOHANG) returned: {}", pid.raw());
                ...
                Ok(pid) => {
                    println!("  FAIL: Unexpected return value {}", pid.raw());
                    println!("WNOHANG_TIMING_TEST_FAILED");
                    std::process::exit(1);
                }
            };
            ...
            if early_wait_ok {
                ...
                match process::waitpid(fork_result, &mut status2, 0) {
                    Ok(pid) if pid.raw() as i32 == fork_result => {
                        println!("  waitpid returned: {}", pid.raw());
