...
//!
//! This is a standalone test binary that duplicates the JobTable types
//! from init_shell.rs since we cannot share code in no_std without
//! restructuring into a separate crate.
...
fn print_num(mut n: u64) {
    ...
    while i > 0 {
        i -= 1;
        let _ = libbreenix::io::write(libbreenix::types::fd::STDOUT, &[buf[i]]);
    }
}
...
fn assert_eq_u32(actual: u32, expected: u32, msg: &str) -> bool {
    ...
    if actual == expected {
        ...
        print(msg);
        print(" - expected ");
        print_num(expected as u64);
        print(", got ");
        print_num(actual as u64);
        println("");
        false
    }
}
...
/// In tests, we'll use a thread-local or pass it explicitly
...
pub extern "C" fn _start() -> ! {
    ...
    unsafe {
        print("Tests run: ");
        print_num(TESTS_RUN as u64);
        println("");
        print("Tests passed: ");
        print_num(TESTS_PASSED as u64);
        println("");
        ...
        if TESTS_PASSED == TESTS_RUN {
            ...
            print("FAILED: ");
            print_num((TESTS_RUN - TESTS_PASSED) as u64);
...
fn panic(_info: &PanicInfo) -> ! {
    println("PANIC in job_table_test!");
    exit(255);
}
