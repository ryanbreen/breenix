...
#
# This script creates an ext2 filesystem image (32MB x86_64, 64MB aarch64) with:
...
if [[ "$ARCH" == "aarch64" ]]; then
    ...
    # ARM64 binaries are larger, use 64MB default (need headroom for user data like BMP saves)
    if [[ "$SIZE_MB" == "32" ]]; then
        SIZE_MB=64
    fi
else
