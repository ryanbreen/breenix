...
impl Ext2Fs {
    ...
    pub fn create_directory(&mut self, path: &str, mode: u16) -> Result<u32, &'static str> {
        ...
        dir_data[dotdot_offset..dotdot_offset + 4].copy_from_slice(&parent_inode_num.to_le_bytes());
        dir_data[dotdot_offset + 4..dotdot_offset + 6].copy_from_slice(&dotdot_rec_len.to_le_bytes());
    ...
    pub fn list_directory(&self, path: &str) -> Result<Vec<DirEntry>, &'static str> {
        let inode_num = self.resolve_path(path)?;
        let inode = self.read_inode(inode_num)?;
        let dir_data = self.read_directory(&inode)?;
        Ok(parse_directory(&dir_data))
    }
