...
mod tests {
    ...
    fn test_backspace() {
        ...
        // Type "helo", erase 'o' and 'l', then retype "llo\n" → "hello\n"
        for c in b"helo" {
            ld.input_char(*c, &mut null_echo);
        }
        ld.input_char(0x7F, &mut null_echo); // DEL (erase 'o') → "hel"
        ld.input_char(0x7F, &mut null_echo); // DEL (erase 'l') → "he"
        for c in b"llo\n" {
            ld.input_char(*c, &mut null_echo);
        }
    ...
    fn test_termios_accessors() {
        ...
        let new_termios = Termios::default();
