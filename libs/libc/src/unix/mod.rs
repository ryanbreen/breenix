...
extern "C" {
    ...
    // Note: POSIX specifies argv/envp as char *const[], but for backwards
    // compatibility with existing code we use *const *const. The pointers
    // themselves can be modified, but the caller typically provides const arrays.
