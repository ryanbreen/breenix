...
cfg_if! {
    if #[cfg(target_os = "aix")] {
        ...
        // Breenix uses its own module in unix/breenix/
        // No new-style module needed
    }
}
...
cfg_if! {
    if #[cfg(all(
        target_family = "unix",
        not(target_os = "qurt"),
        not(target_os = "breenix"),
        // Exclude Linux targets without a standard libc env (e.g., custom OS targets)
        // These modules require glibc/musl/uclibc to be loaded first
        not(all(
            target_os = "linux",
            not(any(target_env = "gnu", target_env = "musl", target_env = "uclibc", target_env = "ohos"))
        ))
    ))] {
