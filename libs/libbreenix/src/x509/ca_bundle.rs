...
//! used to validate TLS certificate chains. Ships with the Mozilla
//! root CA certificates extracted from curl's CA bundle.
...
/// DER-encoded Mozilla root CA certificates (concatenated blob)
static CA_CERTS_DER: &[u8] = include_bytes!("ca-certificates.der");

// Auto-generated index: (offset, length) for each certificate in the DER blob
include!("ca_index.rs");
...
impl RootStore {
    ...
    ///
    /// Returns the full Mozilla root CA certificate store containing
    /// 144 trusted root certificates. The DER blob and index are
    /// generated by `scripts/gen_ca_bundle.py`.
    pub fn mozilla() -> &'static RootStore {
        static STORE: RootStore = RootStore {
            der_data: CA_CERTS_DER,
            index: CA_CERTS_INDEX,
        };
        &STORE
    }
...
mod tests {
    ...
    #[test]
    fn test_mozilla_store_is_populated() {
        let store = RootStore::mozilla();
        // The store should contain the Mozilla root CAs
        assert!(!store.is_empty());
        assert!(store.len() > 100); // Mozilla bundle has 100+ root CAs
        // Should be able to parse the first certificate
    ...
    fn test_find_issuer_nonexistent() {
        let store = RootStore::mozilla();
        let issuer = vec![(vec![2, 5, 4, 3], b"Example CA That Does Not Exist".to_vec())];
    ...
    fn test_get_out_of_bounds() {
        let store = RootStore::mozilla();
        // Valid index should return a cert
        assert!(store.get(0).is_some());
        // Way out of bounds should return None
        assert!(store.get(10000).is_none());
    }
